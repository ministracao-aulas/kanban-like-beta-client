...
$('#sortable-tasks-1').sortable({
	...
	receive: function (event, ui) {
		...
		for (var i = lists.length - 1; i >= 0; i--) {
			//esta linha executa se nao tiver nenhum card na coluna para colocar uma coluna vazia
			if ($(lists[i]).children().length < 1) $(lists[i]).html('');
			document.getElementById('teste').innerHTML = 'de ' + i + ' para ' + $(lists[i]);
//			document.getElementById('teste').innerHTML = document.getElementById('teste').innerHTML + lists[i].id;
		};
	}
});

$('#sortable-tasks-2').sortable({
	connectWith: ".sortable-connected",
	receive: function (event, ui) {
		var lists = $('.sortable-connected');
		for (var i = lists.length - 1; i >= 0; i--) {
